...
chrome.runtime.onMessage.addListener(
  (request: Request, sender, sendResponse) => {
    if (request.type === MessageType.TO_SCRIPT) {
      ...
      let numOfHighlightedElements: number = highlightElements(
...
const highlightElements = (selector: string) => {
  let matchingElements: HTMLCollectionOf<HTMLElement> =
    document.getElementsByTagName(selector) as HTMLCollectionOf<HTMLElement>
